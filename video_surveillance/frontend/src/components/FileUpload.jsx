...
const FileUpload = () => {
  ...
  return (
    <div class="bg-gray-900 p-5 rounded-lg shadow-lg text-center mx-5">
      {message && (
        <div className="mb-5 p-2.5 rounded bg-orange-600 text-white whitespace-pre-wrap text-left pl-5">
          {message}
        </div>
      )}
      {uploading && (
      ...
      <div class="flex flex-wrap justify-center w-100 mr-2.5">
        {preloadedVideos.map((video, index) => (
          <div key={index} className="flex flex-col items-center mb-5 mr-5">
            ...
            <button
              class="mt-2 w-2/5 p-2.5 border-none rounded bg-blue-600 text-white text-lg cursor-pointer hover:bg-blue-700"
      ...
      <div className="mb-5">
        ...
        <textarea
          ...
          className="w-full h-25 resize-y p-2.5 mb-5 border border-gray-800 rounded bg-gray-800 text-white"
        ></textarea>
      </div>
      <div>
        <h2>Function URL:</h2>
